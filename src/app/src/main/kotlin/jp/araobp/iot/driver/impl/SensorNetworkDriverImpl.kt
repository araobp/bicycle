...
class SensorNetworkDriverImpl : ISensorNetworkDriver {
    ...
    * */
    private fun setConfig(baudrate: Int, dataBits: Byte, stopBits: Byte, parity: Byte, flowControl: Byte) {
        ...
        when (dataBits) {
            7.toByte() -> dataBits = D2xxManager.FT_DATA_BITS_7
            8.toByte() -> dataBits = D2xxManager.FT_DATA_BITS_8
            else -> dataBits = D2xxManager.FT_DATA_BITS_8
        }

        when (stopBits) {
            1.toByte() -> stopBits = D2xxManager.FT_STOP_BITS_1
            2.toByte() -> stopBits = D2xxManager.FT_STOP_BITS_2
            else -> stopBits = D2xxManager.FT_STOP_BITS_1
        }

        when (parity) {
            0.toByte() -> parity = D2xxManager.FT_PARITY_NONE
            1.toByte() -> parity = D2xxManager.FT_PARITY_ODD
            2.toByte() -> parity = D2xxManager.FT_PARITY_EVEN
            3.toByte() -> parity = D2xxManager.FT_PARITY_MARK
            4.toByte() -> parity = D2xxManager.FT_PARITY_SPACE
            else -> parity = D2xxManager.FT_PARITY_NONE
        }
